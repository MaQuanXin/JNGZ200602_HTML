       ...
       $.post("/ajax/ajaxfront.php?action="+action+"",
        ...
        , function(data){
            if(data['result']==1)
            {
               ...
               '<div class="cb"></div>'+
               '<div class="comment">'+
               '<span class="com_con">'+item['content']+'</span>'+
               '</div>';
               if(item['reply']!=null && item['reply_time']!=null)
               {
               datahtml = datahtml + 
			    ' <div class="reply_t" style="padding-left: 0px;">'+
               '<span style="width: 100px; display: block; float: left; color:#d17939; margin-left: 3px;">[ 客服回复 ]</span>'+
